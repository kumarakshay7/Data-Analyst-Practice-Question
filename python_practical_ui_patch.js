(() => {
  const SAMPLES = {
    ...
    'PY-P020': ['Sample data: employee_id = [1, 2, 2, 3], name = [A, B, B, C]', 'Expected output: duplicate row for employee B removed'],
    ...
    'PY-P027': ['Sample customers: [1, 2, 3]; orders: customer_id [1, 1, 2], amount [100, 200, 150]', 'Expected output: all 3 customers retained; customer 3 has no order'],
    'PY-P028': ['Sample customers: (1,A), (2,B); orders: (1,100), (1,200), (2,150)', 'Expected output: A = 300, B = 150'],
    'PY-P029': ['Sample customers: [1, 2, 3]; orders: [1, 2]', 'Expected output: customer 3 has no orders'],
    'PY-P030': ['Sample dates: 2026-01-05, 2026-01-20, 2026-02-02; sales: 100, 200, 150', 'Expected output: 2026-01 = 300, 2026-02 = 150'],
    ...
    'PY-P039': ['Sample customer IDs: [1, 2, 2, 3, 3, 3]', 'Expected output: IDs 2 and 3 with their duplicate rows'],
    ...
    'PY-P045': ['Sample salaries: IT = [50000, 80000], HR = [60000, 40000]', 'Expected output: employees with salaries above their department average: B and C'],
    ...
    'PY-P047': ['Sample sales: [100, 120, 150, 180, 200]', 'Expected output: 3-period rolling averages begin at 123.33, then 150, then 176.67'],
    'PY-P048': ['Sample daily sales: [10, 12, 11, 14, 15, 13, 16, 18, 17, 20]', 'Expected output: 7-day rolling average starts at 13.0 on day 7'],
    'PY-P049': ['Sample purchases: customer 1 on 2026-02-01 and 2026-01-10; customer 2 on 2026-03-02 and 2026-02-20', 'Expected output: customer 1 = 2026-01-10; customer 2 = 2026-02-20'],
    ...
    'PY-P053': ['Sample orders: customer 1 = 100+200, customer 2 = 500, customer 3 = 50+100', 'Expected output: customer 1 and customer 2 are above the average customer value'],
    'PY-P054': ['Sample daily sales: [100, 110, 105, 60, 100]', 'Expected output: the 60-sales day is detected as a sudden drop'],
    'PY-P055': ['Sample DataFrame columns: [id, sales]; required columns: [id, sales, date]', 'Expected output: missing column [date]'],
    'PY-P056': ['Sample data: IT sales [100, 300], HR sales [200]', 'Expected output: IT range = 200; HR range = 0'],
    'PY-P057': ['Sample old_value = [10, 20, 30], new_value = [10, 25, 30]', 'Expected output: row where old_value=20 and new_value=25'],
    'PY-P058': ['Sample data: id = [1,2,2,3], name = [A,None,B,C]', 'Expected output: rows=4, duplicate_rows=0, null_cells=1'],
    'PY-P059': ['Sample sales.csv column: sales = [100, 200, 150, 300, 250]', 'Expected output: total sales = 1000. In the real exercise, place these rows in sales.csv before running.'],
  ...
  function patchCurrentQuestion() {
    ...
    if (q && !question.dataset.baseQuestion) {
      question.dataset.baseQuestion = '1';
      question.innerHTML = '🐍 ' + escapeHtml(q[1]);
    }

    meta.innerHTML = '<strong>' + escapeHtml(sample[0].split(':')[0]) + ':</strong> ' + escapeHtml(sample[0].split(':').slice(1).join(':').trim()) + '<br><strong>' + escapeHtml(sample[1].split(':')[0]) + ':</strong> ' + escapeHtml(sample[1].split(':').slice(1).join(':').trim()) + '<br><span>Use this sample data first. You can also test your solution with other valid values.</span>';
